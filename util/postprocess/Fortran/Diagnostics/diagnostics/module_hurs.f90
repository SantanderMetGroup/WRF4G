...
!  rh2=MAX(MIN(rh2, 1.0), 0.0)
  ...
  IF (debg >= 100) THEN
    ...
    PRINT *,'  t2: ',t2(132, 162, 1)
    PRINT *,'  q2: ',q2(132, 162, 1)
    PRINT *,'  psfc: ',psfc(132, 162, 1)
    PRINT *,'  e: ',e(132, 162, 1)
    PRINT *,'  es: ',es(132, 162, 1)
  END IF
...
!  Subroutine to compute rh2 in K following ITS-90 scale
...
! Local
  ...
  REAL(KIND=Rhigh), DIMENSION(dx,dy,dt)                   :: e, lnes, tempd2
...
!!
  ...
  e=e0*exp(Lv/Rv*(1./tkelvin-1./tempd2))
...
!  e=q2*psfc/(Rd/Rv+q2)
  ...
  IF (debg >= 100) THEN
    PRINT *,'  dim/2 values:___'
    PRINT *,'  q2: ',q2(halfdim(dx),halfdim(dy),halfdim(dt))
    PRINT *,'  psfc: ',psfc(halfdim(dx),halfdim(dy),halfdim(dt))
    PRINT *,'  t2: ',t2(halfdim(dx),halfdim(dy),halfdim(dt))	
    PRINT *,'  es: ',exp(lnes(halfdim(dx),halfdim(dy),halfdim(dt)))/100.
    PRINT *,'  tempd2: ',tempd2(halfdim(dx),halfdim(dy),halfdim(dt))
    PRINT *,'  e: ',e(halfdim(dx),halfdim(dy),halfdim(dt))
    PRINT *,'  rh2: ',rh2(halfdim(dx),halfdim(dy),halfdim(dt))
  END IF

  IF (debg >= 100) THEN
    PRINT *,'  162 132 1 values:___'
    PRINT *,'  q2: ',q2(132, 162, 1)
    PRINT *,'  psfc: ',psfc(132, 162, 1)
    PRINT *,'  t2: ',t2(132, 162, 1)	
    PRINT *,'  lnes: ',exp(lnes(132, 162, 1))/100.
    PRINT *,'  tempd2: ',tempd2(132, 162, 1)
    PRINT *,'  e: ',e(132, 162, 1)
    PRINT *,'  rh2: ',rh2(132, 162, 1)
  END IF
  ...
  SUBROUTINE hurs_Bolton(debg, dx, dy, dt, t2, q2, psfc, rh2)
!  Subroutine to compute rh2 following Bolton
...
! Local
  ...
  REAL(KIND=Rhigh), DIMENSION(dx,dy,dt)                   :: es, e2
  REAL  (KIND=Rhigh)                                      :: td2A, td2B
...
! e: vapor pressure in air [Pa]
  ...
  IF (debg >= 75)  PRINT *,'  Computing rh2 following Bolton....'

  es=0.

  es=es_base_bolton*exp((t2-tkelvin)*es_Abolton/(t2-tkelvin+es_Bbolton))
  ...
  IF (debg >= 100) THEN
    ...
    PRINT *,'  dim/2 es: ',es(halfdim(dx),halfdim(dy),halfdim(dt))
