    ...
    try :
        ...
        if job_db.has_wps() :
            ...
            if params.parallel_real == 'yes' :
                ...
                cmd = "ulimit -s unlimited && %s %s" % ( params.parallel_run, real_exe ) 
                ...
                code, output = exec_cmd( "ulimit -s unlimited && %s > %s" % ( real_exe, real_log ) )
        ...
        if params.parallel_wrf == 'yes' :
            cmd = "ulimit -s unlimited && %s %s" % ( params.parallel_run, wrf_exe )                       
            code, output = exec_cmd( cmd )
            if isfile( log_wrf ) :
            ...
            code, output = exec_cmd( "ulimit -s unlimited && %s > %s" % ( wrf_exe, log_wrf ) )
